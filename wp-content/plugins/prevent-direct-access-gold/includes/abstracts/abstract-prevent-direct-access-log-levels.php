...
/**
 * Class PDA_Log_Levels
 */
abstract class PDA_Log_Levels {
	/**
	 * Log Levels
	 *
	...
	public static function get_level_color( $entry ) {
		if ( strpos( $entry, strtoupper( self::INFO ) ) !== false ) {
			return "#f0f8ff";
		} elseif ( strpos( $entry, strtoupper( self::WARNING ) ) !== false ) {
			return "#ffff00";
		} elseif ( strpos( $entry, strtoupper( self::ERROR ) ) !== false ) {
			return "#ff0000";
		} elseif ( strpos( $entry, strtoupper( self::CRITICAL ) ) !== false ) {
			return "#FF0000";
		} else {
			return "#CCCCCC";
		}
